  ...
  return {
    ...
    cenarios: [
      ...
      },
      {
        ...
        titulo: 'Extravio ou entrega incorreta com reembolso',
        probabilidade: clampNumber(8 + (diasAtraso * 3) + (itensCriticos.length * 2), 3, 70, 14),
        impactoFinanceiro: 16000 + (itensCriticos.length * 3200),
        diasCoberturaAtual: clampNumber((principal.quantidade ?? 0) / 10, 0, 15, 1),
        acaoRecomendada: 'Acionar rastreamento dedicado, formalizar reembolso e disparar reentrega imediata.',
        estoqueSegurancaSugerido: clampNumber((principal.quantidade ?? 0) * 0.6 + 28, 8, 350, 42),
        prioridade: 'alta',
      },
      {
        tipo: 'nao_entregue',
        titulo: 'Nao entrega definitiva',
        probabilidade: clampNumber(4 + diasAtraso + itensCriticos.length, 2, 35, 6),
        impactoFinanceiro: 55000 + (itensCriticos.length * 5000),
        diasCoberturaAtual: clampNumber((principal.quantidade ?? 0) / 14, 0, 10, 0),
        acaoRecomendada: 'Ativar contingencia de suprimentos, compra emergencial e redistribuicao da rede.',
        estoqueSegurancaSugerido: clampNumber((principal.quantidade ?? 0) + 60, 20, 500, 120),
        prioridade: 'alta',
      },
      {
        tipo: 'catastrofe',
