...
class _EstoqueScreenState extends State<EstoqueScreen> {
  ...
  List<Map<String, dynamic>> _filtrar(List<Map<String, dynamic>> itens) {
    ...
    if (_termoBusca.isNotEmpty) {
      lista = lista
          .where((i) =>
      i['nome'].toString().toLowerCase().contains(_termoBusca.toLowerCase()))
          .toList();
    }
          ...
          child: ListView(
            ...
            children: [
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _Chip('Todos', 'todos', _filtroStatus,
                            () => setState(() => _filtroStatus = 'todos')),
                    const SizedBox(width: 8),
                    _Chip('Crítico', 'critico', _filtroStatus,
                    ...
                    const SizedBox(width: 8),
                    _Chip('Normal', 'normal', _filtroStatus,
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Quantidade calculada pela IA · requer atenção especial',
                              style: TextStyle(
                                  fontSize: 11, color: AppTheme.purple600),
                            ),
                          ],
                        ),
                      ),
              ...
              if (_filtroStatus != 'essencial_ia') ...[
                const SectionDivider(label: 'Itens primordiais · monitoramento contínuo'),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Item essencial · baixa demanda regular',
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              _BtnPequeno(
                label: recomendado == 0 ? 'Calcular com IA' : 'Repor',
                onTap: () {
                  if (recomendado == 0) {
                    context.read<EstoqueStore>().calcularEstoqueIa(item['id']);
                  }
                },
              ),
            ],
...
class _InfoMini extends StatelessWidget {
  ...
  final String valor;
  final bool destaque;
  const _InfoMini({required this.label, required this.valor, this.destaque = false});
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 4),
          Text('Cálculo gerado pela IA com base no histórico de consumo',
              style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
          const Divider(height: 24),
          _LinhaDetalhe('Quantidade atual', '$atual unidades'),
          _LinhaDetalhe('Recomendado pela IA', '$recomendado unidades',
              destaque: true),
          _LinhaDetalhe(
              'Local ideal', item['local_armazenamento'] ?? 'Não calculado'),
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Este cálculo considera histórico dos últimos 30 dias, '
                        'sazonalidade e cenários de risco mapeados pela IA.',
                    style: const TextStyle(fontSize: 11, color: AppTheme.purple800),
                  ),
                ),
              ],
            ),
